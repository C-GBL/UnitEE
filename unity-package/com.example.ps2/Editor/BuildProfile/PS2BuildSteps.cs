...
namespace Ps2.Editor
{
    ...
    public sealed class PS2StepNativeBuild : IPS2BuildStep
    {
        ...
        private static void BuildGameExecutable(PS2BuildContext ctx, string toolchainFile)
        {
            ...
            // KNOWN GAP, stated loudly rather than shipped quietly. The
            // il2cpp-port tree still links M6's main_ee.cpp, which boots the
            // M6 hello program rather than loading the exported scene and
            // driving UnityEngine.Internal.Runtime.Tick. The ELF therefore
            // builds, links and boots, and then TLB-misses because the
            // generated code it was linked against is not the code that main
            // expects.
            //
            // Everything up to and including the link is real; what is
            // missing is a generic game host main(). Saying so here is the
            // difference between a known gap and a mystery crash on the
            // user's console.
            ctx.Warn(
                "The linked ELF uses the M6 sample host (main_ee.cpp), not a " +
                "generic game host, so it will not run the exported scene yet. " +
                "The build pipeline itself is complete: export, strip, IL2CPP, " +
                "bridge, native link and ISO packaging all produce real output.");
        }
    }
